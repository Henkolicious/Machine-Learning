...
def read_whole_training_images(path):
    ...
    for i in range(len(os.listdir(path))):
        ...
        for image in imagesList:        
                image_raw_data_jpg = tf.gfile.FastGFile(path+os.listdir(path)[i]+'/'+image, 'rb').read() 
                #Decode each image
...
for _ in range(number_of_training):
        for i in range(epochs):
            ...
            idx=np.sort(idx)
            Xtr_training, Ytr_training = next_batch(batch_size, Xtr_Original, Ytr_Original,idx)
            sess.run([cost, optimizer], feed_dict={X: Xtr_training, Y: Ytr_training})

correct_prediction = tf.equal(tf.argmax(hypothesis, 1), tf.argmax(Y, 1))
accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))
print(sess.run(accuracy, feed_dict={X: training_dataset, Y: labels_training}))
